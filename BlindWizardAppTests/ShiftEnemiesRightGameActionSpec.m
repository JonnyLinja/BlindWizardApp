...
describe(@"ShiftEnemiesRightGameAction", ^{
    ...
    beforeEach(^{
        ...
        sut = [[ShiftEnemiesRightGameAction alloc] initWithRow:-1 gameBoard:gameBoardMock factory:factoryMock duration:0];
    });
    ...
    context(@"when generating next game action", ^{
        it(@"should create a checkdangerous, drop, and a destroy game action", ^{
            //context
            OCMExpect([factoryMock checkDangerousGameActionWithBoard:gameBoardMock]).andReturn(sut);
